...
export const loader = async ({ request }) => {
  ...
  // list hotels (basic list)
  const hRes = await admin.graphql(Q_METAOBJECTS_BY_TYPE, {
    variables: { type: "hotel", first: 100 },
  });
  ...
  const hotels = (hJson?.data?.metaobjects?.nodes || []).map((n) => {
    ...
    return {
      ...
      gallery: (() => {
        try { return JSON.parse(F.gallery || "[]"); } catch { return []; }
      })(), // [ID]
    };
  });
...
export const action = async ({ request }) => {
  ...
  if (intent === "save") {
    ...
    const star_rating = form.get("star_rating") || "";
    const hero_image = form.get("hero_image") || ""; // single ID
    const gallery = JSON.parse(form.get("gallery") || "[]"); // array of IDs

    const handle = `hotel-${slugify(name) || Date.now()}`;
    ...
    const up = await gqlAdmin(admin, M_METAOBJECT_UPSERT_GENERIC, {
      ...
      fields: [
        ...
        ...(star_rating ? [{ key: "star_rating", value: String(star_rating) }] : []),
...
export default function HotelsManager() {
  ...
  const [editing, setEditing] = React.useState({
    ...
    hero_image: "",     // ID
    gallery: [],        // [ID]
    _mediaNodes: [],    // [{id, preview, filename}] for immediate previews
  });
  ...
  // Build a map for quick preview lookup
  ...
  const galleryNodes = editing.gallery
    .map((id) => mediaMap.get(id))
    .filter(Boolean);

  return (
    <Page>
      <TitleBar title="Hotels" />
      <Layout>
        {/* Left: List */}
        <Layout.Section>
          <Card>
            <BlockStack gap="400">
              ...
              {hotels.length === 0 ? (
                <Banner tone="info">No hotels yet.</Banner>
              ) : (
                <Box padding="300">
                    ...
                    {hotels.map((h) => (
                      <li key={h.id} style={{ marginBottom: 8 }}>
                        <InlineStack gap="300" align="start">
                          ...
                          <Button size="slim" onClick={() => loadItem(h.id)}>
                            Edit
                          </Button>
                          <fetcher.Form
                            method="post"
                            onSubmit={(e) => {
                              if (!confirm("Delete this hotel?")) e.preventDefault();
                            }}
                          >
          ...
          <Card>
            <BlockStack gap="400">
              <Text as="h2" variant="headingMd">
                {editing.id ? "Edit hotel" : "New hotel"}
              </Text>
              ...
              <InlineStack gap="300" align="start">
                ...
                <TextField
                  label="City"
                  value={editing.city}
                  onChange={(v) => setEditing((s) => ({ ...s, city: v }))}
                />
                <TextField
                  label="Star rating (1-5)"
                  type="number"
                  value={String(editing.star_rating)}
                  onChange={(v) => setEditing((s) => ({ ...s, star_rating: v }))}
                />
              </InlineStack>
              ...
              <InlineStack gap="300">
                <fetcher.Form method="post">
                  ...
                  <input type="hidden" name="city" value={editing.city} />
                  <input type="hidden" name="description" value={editing.description} />
                  <input type="hidden" name="star_rating" value={editing.star_rating} />
                  ...
                  <Button variant="primary" submit disabled={!editing.name} loading={isBusy}>
                    Save
                  </Button>
                </fetcher.Form>
      ...
      <MediaPicker
        ...
        multiple
        initialSelected={
          // seed the picker with currently known nodes (so it shows as selected)
        ...
        onConfirm={({ heroId, galleryIds, nodes }) => {
          setEditing((s) => ({
            ...
            _mediaNodes: nodes?.map((n) =>
              // normalize in case picker returns {image:{url}} instead of {preview}
